...
const navItems = [
  ...
  { href: "/admin/series", label: "Series", icon: PackageCheck },
...
export function AdminShell({ children }: { children: React.ReactNode }) {
    ...
    if (href === "/admin/series") {
      return (
        ...
        (pathname.startsWith("/admin/series/") &&
          !pathname.startsWith("/admin/series/compras"))
      );
    }
  ...
  return (
    <div className="min-h-screen bg-[#f7f1ea] md:grid md:grid-cols-[300px_1fr]">
        ...
        <div className="relative flex h-full flex-col px-5 pb-6 pt-6">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {active && <span className="ml-auto size-2 rounded-full bg-white" />}
                </Link>
              );
            })}
          </nav>
          ...
          <div className="rounded-t-[24px] bg-[#7a3428]/80 px-4 pb-2 pt-4 backdrop-blur-sm">
            <div className="rounded-[18px] border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-3 text-xs text-[#f2d6c6]">admin@tendenciasimport.pe</p>
            </div>

            <form action={logout} className="mt-4">
              <button className="flex w-full items-center gap-3 rounded-[20px] bg-[#fff8f1] px-4 py-3 font-black text-[#8f3a2e] shadow-sm transition hover:bg-[#fff1e7]">
                <span className="grid size-9 place-items-center rounded-full bg-[#f8ece2] text-[#8f3a2e]">
                  <LogOut size={17} />
                </span>
      ...
      <main className="min-w-0 bg-[#f7f1ea] p-4 md:p-7">{children}</main>
    </div>
  );
}
